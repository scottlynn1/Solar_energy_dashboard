...
optimizeoutput.addEventListener('click', (e) => {
  ...
  fetch(`optimize?${[params]}`, {
    ...
    AnnualData.optimizeddata.textContent = `Tilt: ${Math.round(returndata.optimal_tilt)}  Azimuth: ${returndata.optimal_azimuth} with ${returndata.ac_annual} kWh AC annual production`;
  }).catch(error => {
    loadingsign.className = 'loadinghide';
    optimizeoutput.hidden = false;
